...
class SwmmInputEncoder:
    ...
    def _write_landuses(self, model: Dict[str, Any], file: TextIO):
        ...
        The engine accepts a name alone or all four columns and refuses
        anything in between (ERROR 203, too few items), so a row with any
        sweeping field is written with all three, missing ones as 0. A
        ``percent_imperv`` left in a data.json by the earlier reading was
        the sweep interval, and is written as such.
        """
        if "landuses" in model and model["landuses"]:
            ...
            for landuse in model["landuses"]:
                ...
                interval = self._get_field(landuse, "sweep_interval", "percent_imperv", default=None)
                if interval is None:
                    file.write(f"{name}\n")
                    continue
                availability = self._get_field(landuse, "availability", default="0")
                last_swept = self._get_field(landuse, "last_swept", default="0")
                file.write(f"{name:<16} {interval:<10} {availability:<10} {last_swept}\n")
